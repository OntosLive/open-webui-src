<script lang="ts">
	...
	import { PORTAL, PROJECT } from '$lib/config/branding';
	...
	<div class=" space-y-3 overflow-y-scroll max-h-[28rem] md:max-h-full">
		...
		{#if $config?.license_metadata}
			<div class="mb-2 text-xs">
				<span class=" text-gray-500 dark:text-gray-300 font-medium">{PORTAL.name}</span> -
		...
		<div>
			<pre
				class="text-xs text-gray-400 dark:text-gray-500">Copyright (c) {new Date().getFullYear()} <a
					href="https://ontos.live"
					target="_blank"
					class="underline">{PORTAL.name}</a
